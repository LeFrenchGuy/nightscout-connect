...
// --- Main driver factory ---
// Authentication upgraded from SSO/cookie-based to OAuth Bearer tokens.
// Requires a logindata.json file generated by carelink-bridge or similar tool.
// The logindata.json contains OAuth tokens (access_token, refresh_token)
// and the token endpoint URL for automatic refresh.
...
function carelinkSource (opts, axios) {
  ...
  // Persist login data state across authenticate/refresh calls
  var _loginData = null;
  var _loginDataPath = null;

  var impl = {

    // OAuth authentication: load logindata.json and refresh token if needed.
    // Returns auth info with Bearer token for subsequent API calls.
    authFromCredentials (creds, settings) {

      var loaded = oauth.loadLoginData(opts.carelinkLoginData);
      if (!loaded.data) {
        return Promise.reject(new Error(
          'Cannot load logindata.json. Set CONNECT_CARELINK_LOGIN_DATA to the file path or inline JSON. '
          + 'Generate it using carelink-bridge or similar tool.'
        ));
      }

      _loginData = loaded.data;
      _loginDataPath = loaded.filePath;
      ...
      if (oauth.isTokenExpired(_loginData.access_token)) {
        ...
        return oauth.refreshAccessToken(_loginData, axios)
          .then(function (refreshed) {
            ...
            oauth.saveLoginData(_loginDataPath, refreshed);
            return {
              token: refreshed.access_token,
              loginData: refreshed,
              loginDataPath: _loginDataPath,
            };
          })
          .catch(function (error) {
            ...
            return Promise.reject(new Error(
              ...
              + 'Regenerate logindata.json using carelink-bridge.'
            ));
          });
      }
    ...
    refreshSession (authInfo, session) {
      ...
      return oauth.refreshAccessToken(loginData, axios)
        .then(function (refreshed) {
          _loginData = refreshed;
          oauth.saveLoginData(authInfo.loginDataPath || _loginDataPath, refreshed);
...
//
// Required:
//   CONNECT_CARELINK_LOGIN_DATA - Path to logindata.json or inline JSON string
//                                 (generated by carelink-bridge)
//   CONNECT_COUNTRY_CODE        - Two-letter country code (auto-detected from JWT if omitted)
//
// Optional:
//   CONNECT_CARELINK_REGION          - 'us' or 'eu' (auto-detected from token_url if omitted)
...
carelinkSource.validate = function validate_inputs (input) {
  ...
  // Default logindata.json path if not specified
  var loginDataPath = input.carelinkLoginData || require('path').join(process.cwd(), 'logindata.json');

  // Load logindata.json once for validation and auto-detection
  var loaded = oauth.loadLoginData(loginDataPath);

  // Auto-detect region from token_url in logindata.json
  var detectedRegion = null;
  if (loaded && loaded.data) {
    detectedRegion = oauth.detectRegion(loaded.data);
  }

  var config = {
    carelinkRegion: input.carelinkRegion || detectedRegion,
    carelinkServer: input.carelinkServer,
    carelinkLoginData: loginDataPath,
  ...
  if (!loaded || !loaded.data) {
    errors.push({desc: "CareLink login data not found at " + loginDataPath + ". Use the admin page or run 'nightscout-connect login' to authenticate with CareLink.", err: new Error('CONNECT_CARELINK_LOGIN_DATA') } );
  }

  // Auto-detect country code from JWT payload if not explicitly provided
  if (!config.countryCode && loaded && loaded.data) {
    var payload = oauth.decodeJwtPayload(loaded.data.access_token);
    if (payload && payload.token_details && payload.token_details.country) {
      config.countryCode = payload.token_details.country.toLowerCase();
      console.log('[OAuth] Auto-detected country code from JWT:', config.countryCode);
    }
  }
